...
// send 1 byte to controller and read 1 byte back
uint8_t ps2_transfer(uint8_t out)
{
   uint8_t in = 0;
   for (int i = 0; i < 8; i++) {
       // set CMD bit
       if (out & (1 << i)) {
           HAL_GPIO_WritePin(PS2_PORT, PS2_CMD_PIN, GPIO_PIN_SET);
       } else {
           HAL_GPIO_WritePin(PS2_PORT, PS2_CMD_PIN, GPIO_PIN_RESET);
       }
       // clock low
       HAL_GPIO_WritePin(PS2_PORT, PS2_CLK_PIN, GPIO_PIN_RESET);
       ps2_delay();
       // read DAT
       if (HAL_GPIO_ReadPin(PS2_PORT, PS2_DAT_PIN) == GPIO_PIN_SET) {
           in |= (1 << i);
       }
       // clock high
...
// read controller buttons (2 bytes)
void ps2_read_buttons(uint8_t *b1, uint8_t *b2)
{
   // select controller
   HAL_GPIO_WritePin(PS2_PORT, PS2_ATT_PIN, GPIO_PIN_RESET);
   ps2_delay();
   (void)ps2_transfer(0x01);
   (void)ps2_transfer(0x42);   // poll
   (void)ps2_transfer(0x00);   // status
   uint8_t data1 = ps2_transfer(0x00);
   uint8_t data2 = ps2_transfer(0x00);
   // deselect
   HAL_GPIO_WritePin(PS2_PORT, PS2_ATT_PIN, GPIO_PIN_SET);
   if (b1) *b1 = data1;
   if (b2) *b2 = data2;
}
// active-low buttons: 0 = pressed
void handle_ps2_move(uint8_t b1, uint8_t b2)
{
   // common PS2 mapping:
   // b1 bit4 = UP, bit7 = DOWN, bit6 = LEFT, bit5 = RIGHT
   if (!(b1 & (1 << 4))) { // UP
...
// repaint whole grid red, then cursor green
void drawGridWithCursor(void)
{
   for (int y = 0; y < 16; y++) {
       for (int x = 0; x < 16; x++) {
           setPixel(x, y, 200, 0, 0); // red
       }
   }
   setPixel(cursorX, cursorY, 0, 200, 0); // green
}
/* USER CODE END 0 */
/**
...
int main(void)
{
 ...
 //int currentColor = 0; //0 = red, 1 = green, 2 = blue
  drawGridWithCursor(); //new
 showLeds(); //new
 uint8_t b1_last = 0xFF, b2_last = 0xFF; // new
 while (1)
 {
   /* USER CODE END WHILE */
	 // showLogo();
	 ...
	 // HAL_Delay(200);
     uint8_t b1, b2;
     ps2_read_buttons(&b1, &b2);
     // if something changed, update cursor + LEDs
     if (b1 != b1_last || b2 != b2_last) {
         handle_ps2_move(b1, b2);
         drawGridWithCursor();
         showLeds();
     }
     b1_last = b1;
     b2_last = b2;
     HAL_Delay(30);   // poll every 30 ms
	  //original 3 lines ^
   /* USER CODE BEGIN 3 */
 }
 /* USER CODE END 3 */
...
void showLogo(){
 ...
 setPixel(0, 1, 0, 0, 234);
 setPixel(1, 0, 0, 0, 234);
 setPixel(0, 15, 0, 0, 234);
 setPixel(15, 0, 0, 0, 234);
}
/**
...
static void MX_GPIO_Init(void)
{
 ...
 // Outputs: CMD, ATT, CLK
 GPIO_InitStruct2.Pin = PS2_CMD_PIN | PS2_ATT_PIN | PS2_CLK_PIN;
 GPIO_InitStruct2.Mode = GPIO_MODE_OUTPUT_PP;
 GPIO_InitStruct2.Pull = GPIO_NOPULL;
 GPIO_InitStruct2.Speed = GPIO_SPEED_FREQ_LOW;
 HAL_GPIO_Init(PS2_PORT, &GPIO_InitStruct2);
 // Input: DAT
 GPIO_InitStruct2.Pin = PS2_DAT_PIN;
 GPIO_InitStruct2.Mode = GPIO_MODE_INPUT;
 GPIO_InitStruct2.Pull = GPIO_NOPULL; // controller has internal pull-up
 HAL_GPIO_Init(PS2_PORT, &GPIO_InitStruct2);
 // Idle levels
 HAL_GPIO_WritePin(PS2_PORT, PS2_ATT_PIN, GPIO_PIN_SET); // not selected
 HAL_GPIO_WritePin(PS2_PORT, PS2_CLK_PIN, GPIO_PIN_SET); // clock high
 HAL_GPIO_WritePin(PS2_PORT, PS2_CMD_PIN, GPIO_PIN_SET); // line high
